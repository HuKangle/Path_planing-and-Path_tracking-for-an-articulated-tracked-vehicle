function [x,y,th,D,delta] = New_HybridAStar226(Start,End,Vehicle,Configure)  
    veh = Vehicle;
    cfg = Configure;
    mres = cfg.MOTION_RESOLUTION; % motino resolution 
    
    % 把起始的位姿(x,y,theta)转换为grid上的栅格索引
    [isok,xidx,yidx,thidx,psidx] = CalcIdx(Start(1),Start(2),Start(3),0,cfg);
    if isok % 把位姿栅格定义为一个结点，形成链表结构
        tnode = New_Node(xidx,yidx,thidx,mres,0,Start(1),Start(2),Start(3),[xidx,yidx,thidx],0);
        tnode.yaw_idx = psidx;
        tnode.psi = Start(3);
%         tnode.gamma = 0;
    end
...
%         if sqrt((End(1)-wknode.x)^2+(End(2)-wknode.y)^2)<=20
...
function [x,y,th,D,delta] = getFinalPath(path,Close,veh,cfg)
    ...
    % 固定值31，和栅格的索引是选取线的交点还是选取栅格中心有关，floor是朝负无穷大四舍五入    
    % 每条轨迹大概是2米的长度。这里乘以1.5是确保下一个末端状态肯定在另一个栅格中，
    %不会还在一个栅格中！在地图栅格中子结点拓展。比如对角线长度是1.4，此时还是在同一个栅格中
    x = [];
    y = [];
    th = [];
    D = [];
    ...
    if length(nodes) >= 2
        ...
        for i = length(nodes):-1:2
            ...
            for idx = 1:nlist
                [px,py,pth] = VehicleDyn(px,py,pth,ttnode.D,ttnode.delta,tnode.delta,veh.WB);
                x = [x, px];
                y = [y, py];
                th = [th, pth];
                D = [D, ttnode.D];
    ...
    for i = 1:5
        ...
        for idx = 1:round(abs(segs(i))/mres) % 四舍五入为最近的小数或整数
           	[px,py,pth] = VehicleATVDyn(px,py,pth,s*mres,tdelta,veh.WB,veh.MIN_CIRCLE); % s*mres中s代表前进和后退
            x = [x, px];
            y = [y, py];
            th = [th, pth];
            D = [D, s*mres];
...
function [isok,tnode] = CalcNextNode(wknode,D,delta,veh,cfg)
    ...
    for idx = 1:nlist % 根据当前的状态和给定的控制，计算此条路径上的连着的车辆状态，根据上一时刻计算下一时刻
        [px,py,pth,pga] = VehicleDyn(px,py,pth,D,wknode.delta,delta,veh.WB);
        x(idx+1) = px; % x,y,th储存了数据，但是没用到变量
        y(idx+1) = py;
        th(idx+1) = pth;
        ga(idx+1) = pga;
        if rem(idx,4) == 0 % 每隔5个点进行一次碰撞检测; 现在改成每隔4点进行检测；
            tvec = [px,py,pth,pga];
            isCollision = New_VehicleCollisionCheck(tvec,obstline,veh);
            if isCollision
                break
            end
        end
...
%         plot(x,y);
        [isok,xidx,yidx,thidx,psidx] = CalcIdx(px,py,pth,pga,cfg); % 把路径末端点的实际坐标转换为栅格坐标
        if isok == false
            return
        else
            cost = wknode.cost;
            ...
            tnode = New_Node(xidx,yidx,thidx,D,delta,px,py,pth,...
    ...
    for i = 1:5
        ...
        for idx = 1:round(abs(segs(i))/mres) % round()四舍五入
            % D和delta是固定，说明转弯的时候是按固定半径的圆转弯
           	[px,py,pth,pga] = VehicleATVDyn(px,py,pth,D,delta,veh.WB,veh.MIN_CIRCLE);
...
function [x,y,theta,gamma] = VehicleDyn(x,y,theta,D,past_delta,delta,L)
    x = x+D*cos(theta); % 运动学公式： x_dot = v_x * cos(theta); x_dot * t = v_x * t * cos(theta),在采样时间t内,则有x = x + v_x * t * cos(theta)，其中v_x * t=D
    y = y+D*sin(theta); % 运动学公式
    gamma = delta;
    theta = theta+(D*sin(gamma))/L+(delta-past_delta)*0.2; % L是轴距,航向变化,theta_dot=v/R,R=L/tan(delta)
%     theta = theta+(D*sin(gamma))/L;
    theta = mod2pi(theta);
    gamma = mod2pi(gamma);
end
function [x,y,theta,gamma] = VehicleATVDyn(x,y,theta,D,delta,L,rmin)
%     data_x = D*sin(alpha);
%     data_y = D*(1-cos(alpha));
%     x = x+data_x*cos(theta)-data_y*sin(theta); % 运动学公式： x_dot = v_x * cos(theta); x_dot * t = v_x * t * cos(theta),在采样时间t内,则有x = x + v_x * t * cos(theta)，其中v_x * t=D
%     y = y+data_x*sin(theta)+data_y*cos(theta); % 运动学公式
    x = x+D*cos(theta); % 运动学公式： x_dot = v_x * cos(theta); x_dot * t = v_x * t * cos(theta),在采样时间t内,则有x = x + v_x * t * cos(theta)，其中v_x * t=D
    y = y+D*sin(theta); % 运动学公式
    theta = theta+(D*sin(delta))/L; % L是轴距,航向变化,theta_dot=v/R,R=L/tan(delta)
    theta = mod2pi(theta);
    gamma = mod2pi(delta);
end
% 把位姿(x,y,theta)转换为grid上的栅格索引,如果不符合实际则isok=false
function [isok,xidx,yidx,thidx,psidx] = CalcIdx(x,y,theta,gamma,cfg)
    gres = cfg.XY_GRID_RESOLUTION;
    yawres = cfg.YAW_GRID_RESOLUTION;
    xidx = ceil((x-cfg.MINX)/gres);
    yidx = ceil((y-cfg.MINY)/gres);
    theta = mod2pi(theta); % 控制theta范围在[-pi,pi]区间
    psi = mod2pi(theta-gamma);
    thidx = ceil((theta-cfg.MINYAW)/yawres);
    psidx = ceil((psi-cfg.MINYAW)/yawres);
    isok = true;
