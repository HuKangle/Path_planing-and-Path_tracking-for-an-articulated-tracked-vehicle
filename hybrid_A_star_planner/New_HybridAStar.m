function [x,y,th,D,delta] = New_HybridAStar(Start,End,Vehicle,Configure)  
    veh = Vehicle;
    cfg = Configure;
...
%     end
    ...
    while ~isempty(Open)
        ...
        if sqrt((End(1)-wknode.x)^2+(End(2)-wknode.y)^2)<=10
...
function [x,y,theta,gamma] = VehicleDyn(x,y,theta,D,past_delta,delta,L)
    ...
    y = y+D*sin(theta); % 运动学公式
    gamma = delta;
%     theta = theta+(D*sin(gamma))/L+(delta-past_delta)*2.23/L; % L是轴距,航向变化,theta_dot=v/R,R=L/tan(delta)
